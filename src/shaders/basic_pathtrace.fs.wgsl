...
fn ray_voxel_intersection(ray_origin: vec3<f32>, ray_dir: vec3<f32>) -> vec3<i32> {
    ...
    var t_max = (vec3<f32>(voxel + max(step, vec3<i32>(0))) * VOXEL_SIZE - ray_origin) / ray_dir;  // Changed to var
...
fn fs_main(@builtin(position) pos: vec4<f32>) -> @location(0) vec4<f32> {
    let uv = (pos.xy / vec2<f32>(640.0, 480.0)) * 2.0 - 1.0;

    let ray_origin = uniforms.camera_pos;
    let ray_dir = normalize(uniforms.camera_front + 
                            uv.x * uniforms.camera_right * uniforms.aspect_ratio * uniforms.fov +
                            uv.y * uniforms.camera_up * uniforms.fov);

    var seed = u32(pos.x + pos.y * 640.0 + uniforms.time * 1000.0);
